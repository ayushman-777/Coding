...
Node* constructTree(int pre[], int n, int &ind, int key, int min, int max) {
    ...
    if (min < key && max > key) {
        ...
        ind++;
        if (ind < n)
            node->left = constructTree(pre, n, ind, pre[ind], min, key);
        if (ind < n)
            node->right = constructTree(pre, n, ind, pre[ind], key, max);
    }
    return node;
}
...
{
    int ind = 0;
    return constructTree(pre, size, ind, pre[ind], INT_MIN, INT_MAX);
}
